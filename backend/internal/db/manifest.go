...
func (db *DB) GetManifestEntry(ctx context.Context, jobID int64, relPath string) (*ManifestEntry, error) {
	row := db.QueryRowContext(ctx,
		`SELECT id, job_id, rel_path, sha256, hash_algo, size_bytes, mod_time, permissions, synced_at
		 FROM manifest_entries WHERE job_id = ? AND rel_path = ?`,
		jobID, relPath,
	)
	e := &ManifestEntry{}
	var perm int64
	err := row.Scan(&e.ID, &e.JobID, &e.RelPath, &e.ContentHash, &e.HashAlgo, &e.SizeBytes, &e.ModTime, &perm, &e.SyncedAt)
...
func (db *DB) UpsertManifestEntry(ctx context.Context, e *ManifestEntry) error {
	_, err := db.ExecContext(ctx,
		`INSERT INTO manifest_entries (job_id, rel_path, sha256, hash_algo, size_bytes, mod_time, permissions, synced_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		...
		e.JobID, e.RelPath, e.ContentHash, e.HashAlgo, e.SizeBytes, e.ModTime, int64(e.Permissions), e.SyncedAt,
	)
	return err
}
...
func (db *DB) ListManifestEntries(ctx context.Context, jobID int64) ([]*ManifestEntry, error) {
	...
	for rows.Next() {
		e := &ManifestEntry{}
		var perm int64
		if err := rows.Scan(&e.ID, &e.JobID, &e.RelPath, &e.ContentHash, &e.HashAlgo, &e.SizeBytes, &e.ModTime, &perm, &e.SyncedAt); err != nil {
			return nil, err
		}
		e.Permissions = fs.FileMode(perm)
